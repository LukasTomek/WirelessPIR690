...
_00134_DS_:
	BTFSS	STATUS,0
	GOTO	_00106_DS_
;;genSkipc:3257: created from rifx:0x7ffe804773d0
...
_00251_DS_:
	...
	GOTO	_00155_DS_
;;genSkipc:3257: created from rifx:0x7ffe804773d0
;	.line	225; "main.c"	bitfield.Hint = TRUE;
	BANKSEL	_bitfield
	BSF	_bitfield,2
;;swapping arguments (AOP_TYPEs 1/3)
...
_00155_DS_:
;	.line	245; "main.c"	if (i > 8 && bitfield.Hint && k < 10){
	MOVLW	0x09
	BANKSEL	r0x1071
	SUBWF	r0x1071,W
	BTFSS	STATUS,0
...
;	.line	248; "main.c"	k++;
	BANKSEL	r0x1074
	INCF	r0x1074,F
;	.line	249; "main.c"	temp = TMR0;
	BANKSEL	_TMR0
	MOVF	_TMR0,W
	BANKSEL	r0x1072
	MOVWF	r0x1072
;	.line	251; "main.c"	SP_send(strw);
	MOVLW	high (_strw + 0)
	MOVWF	r0x1075
	MOVLW	(_strw + 0)
	MOVWF	r0x1073
	...
	CALL	_SP_send
	PAGESEL	$
;	.line	253; "main.c"	uint8_to_ascii(temp);
	BANKSEL	r0x1072
	MOVF	r0x1072,W
	PAGESEL	_uint8_to_ascii
	CALL	_uint8_to_ascii
	PAGESEL	$
;	.line	254; "main.c"	SP_send(enter);
	MOVLW	high (_enter + 0)
	BANKSEL	r0x1073
	MOVWF	r0x1073
	MOVLW	(_enter + 0)
	...
	PAGESEL	$
_00160_DS_:
;	.line	256; "main.c"	GIE = 1;
	BANKSEL	_INTCONbits
	BSF	_INTCONbits,7
;	.line	257; "main.c"	t1 = t;
	BANKSEL	_t
	MOVF	_t,W
	BANKSEL	_t1
	MOVWF	_t1
...
;	.line	260; "main.c"	if(i >= 10){
	...
	GOTO	_00167_DS_
;;genSkipc:3257: created from rifx:0x7ffe804773d0
;	.line	261; "main.c"	for(j = 0; j < 10; j++){
	CLRF	r0x1072
_00171_DS_:
;	.line	262; "main.c"	dec_to_ascii(bw[j]);
	MOVLW	0x02
	MOVWF	STK00
	BANKSEL	r0x1072
	MOVF	r0x1072,W
...
;	.line	263; "main.c"	SP_send(tab);
	...
	CALL	_SP_send
	PAGESEL	$
;	.line	264; "main.c"	dec_to_ascii(bt[j]);
	BANKSEL	r0x1073
	MOVF	r0x1073,W
	ADDLW	(_bt + 0)
	MOVWF	r0x1073
...
_SP_send_error:
; 2 exit points
;	.line	364; "main.c"	void SP_send_error(char str[])
	BANKSEL	r0x104F
	MOVWF	r0x104F
	MOVF	STK00,W
	MOVWF	r0x1050
	MOVF	STK01,W
	MOVWF	r0x1051
;	.line	367; "main.c"	GIE = FALSE;	// Disable all interrupts.
	BANKSEL	_INTCONbits
	BCF	_INTCONbits,7
;	.line	368; "main.c"	for(i=0; str[i] != '\0'; i++)
	BANKSEL	r0x1052
	CLRF	r0x1052
_00328_DS_:
	BANKSEL	r0x1052
	...
	BTFSC	STATUS,2
	GOTO	_00330_DS_
;	.line	370; "main.c"	TXREG=str[i];	// Add a character to the output buffer
	MOVF	r0x1056,W
	BANKSEL	_TXREG
	MOVWF	_TXREG
_00323_DS_:
;	.line	371; "main.c"	while(!TXIF);	// Wait while the output buffer is full
	BANKSEL	_PIR1bits
	BTFSS	_PIR1bits,4
	GOTO	_00323_DS_
;	.line	368; "main.c"	for(i=0; str[i] != '\0'; i++)
...
_SP_send:
; 2 exit points
;	.line	356; "main.c"	void SP_send(char str[]){
	BANKSEL	r0x105C
	MOVWF	r0x105C
	MOVF	STK00,W
	MOVWF	r0x105D
	MOVF	STK01,W
	MOVWF	r0x105E
;	.line	358; "main.c"	for(i=0; str[i] != '\0'; i++)
	CLRF	r0x105F
_00316_DS_:
	BANKSEL	r0x105F
	MOVF	r0x105F,W
	...
	BTFSC	STATUS,2
	GOTO	_00318_DS_
;	.line	360; "main.c"	circBufPush(str[i]);
	MOVF	r0x1063,W
...
_uint8_to_ascii:
; 2 exit points
;	.line	338; "main.c"	void uint8_to_ascii(uint8_t dec)
	BANKSEL	r0x1064
	MOVWF	r0x1064
;	.line	343; "main.c"	for (i=0 ; dec!=0  || i == 0; i++)
	CLRF	r0x1065
_00302_DS_:
	MOVLW	0x00
	BANKSEL	r0x1064
	...
	GOTO	_00309_DS_
_00301_DS_:
;	.line	345; "main.c"	number[i]=(dec % 10);
	BANKSEL	r0x1065
	MOVF	r0x1065,W
...
_00297_DS_:
	...
	BTFSC	STATUS,2
	GOTO	_00304_DS_
;	.line	351; "main.c"	circBufPush(number[i]);	// Add a character to the output buffer
	MOVF	r0x1064,W
	ADDLW	(_uint8_to_ascii_number_1_34 + 0)
	MOVWF	r0x1065
	MOVLW	high (_uint8_to_ascii_number_1_34 + 0)
...
_dec_to_ascii:
; 2 exit points
;	.line	302; "main.c"	void dec_to_ascii(unsigned short dec)
	BANKSEL	r0x1069
	MOVWF	r0x1069
	MOVF	STK00,W
	MOVWF	r0x106A
;	.line	307; "main.c"	for (i=0 ; dec!=0; i++)
	CLRF	r0x106B
_00282_DS_:
	BANKSEL	r0x106A
	MOVF	r0x106A,W
	...
	BTFSC	STATUS,2
	GOTO	_00291_DS_
;	.line	309; "main.c"	number[i]=(dec % 10);
	MOVF	r0x106B,W
...
_00278_DS_:
	...
	BTFSC	STATUS,2
	GOTO	_00284_DS_
;	.line	315; "main.c"	circBufPush(number[i]);	// Add a character to the output buffer
	MOVF	r0x106A,W
	ADDLW	(_dec_to_ascii_number_1_30 + 0)
	MOVWF	r0x1069
	MOVLW	high (_dec_to_ascii_number_1_30 + 0)
...
;	.line	279; "main.c"	if (next == buf_tail){
	...
	BTFSS	STATUS,2
	GOTO	_00271_DS_
;	.line	280; "main.c"	SP_send_error(buf_err);
...
;	  825+  263 =  1088 instructions ( 2702 byte)
